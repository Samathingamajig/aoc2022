...
def calc_score(blueprint: Blueprint, initial_data: Data) -> int:
    ...
    max_ore_obsidian_same_geodes_bots_best_day: dict[tuple, int] = {}
    max_ore_obsidian_same_day_bots_best_geodes: dict[tuple, int] = {}
    max_ore_obsidian_same_day_geodes_best_bots: dict[tuple, int] = {}
    ...
    for day in range(1, 2):
        while len(queue):
            ...
            if (
                current_data.ore_robots >= blueprint.geode_cost_ore
                and current_data.obsidian_robots >= blueprint.geode_cost_obsidian
            ):
                cont = False
                if (
                    max_ore_obsidian_same_geodes_bots_best_day.get(
                        (current_data.geode, current_data.geode_robots), -1
                    )
                    or max_ore_obsidian_same_day_bots_best_geodes.get(
                        (current_data.time_remaining, current_data.geode_robots), -1
                    )
                    >= current_data.geode
                    or max_ore_obsidian_same_day_geodes_best_bots.get(
                        (current_data.time_remaining, current_data.geode), -1
                    )
                    >= current_data.geode_robots
                ):
                    cont = True
                max_ore_obsidian_same_geodes_bots_best_day[
                    (current_data.geode, current_data.geode_robots)
                ] = current_data.time_remaining
                max_ore_obsidian_same_day_bots_best_geodes[
                    (current_data.time_remaining, current_data.geode_robots)
                ] = current_data.geode
                max_ore_obsidian_same_day_geodes_best_bots[
                    (current_data.time_remaining, current_data.geode)
                ] = current_data.geode_robots
                if cont:
                    continue
            ...
            while len(possibilities(current_data, blueprint)) == 1:
                # print("one")
                current_data.ore += current_data.ore_robots
                current_data.clay += current_data.clay_robots
                current_data.obsidian += current_data.obsidian_robots
                current_data.geode += current_data.geode_robots
            ...
            for purchase in possibilities(current_data, blueprint, True):
                ...
                if (
                    new_data.geode
                    + (current_data.geode_robots * current_data.time_remaining)
                    + (current_data.time_remaining * (current_data.time_remaining - 1))
                    // 2
                    < best_geodes
                ):
                    continue
                ...
                if (
                    new_data.ore_robots > blueprint.geode_cost_ore
                    and new_data.obsidian_robots > blueprint.geode_cost_obsidian
                ):
                    continue
